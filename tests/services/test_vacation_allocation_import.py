...
def test_creates_new_allocation(db, make_employee):
    ...
    result = import_vacation_allocations_from_csv(_csv(2021, "user1@test.local,20"), db)
...
def test_unknown_employee_fails_row(db):
    result = import_vacation_allocations_from_csv(_csv(2021, "ghost@test.local,20"), db)
...
def test_duplicate_employee_in_file_fails_both_rows(db, make_employee):
    make_employee("user1@test.local", "x")
    result = import_vacation_allocations_from_csv(
        _csv(2021, "user1@test.local,20", "user1@test.local,25"), db
    )
    assert result.created == 0
